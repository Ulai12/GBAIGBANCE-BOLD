...
export function NotificationSettingsScreen({ onBack, onToast }: NotificationSettingsScreenProps) {
  ...
  useEffect(() => {
    ...
    fetchNotificationPreferences(user.id)
      .then(setPrefs)
      .catch(() => onToast({ message: t('settings', 'notifSettings.loadError'), type: 'error' }))
  ...
  const handleToggle = async (key: keyof NotificationPreferences) => {
    ...
    try {
      ...
      setPrefs({ ...prefs, [key]: !newValue });
      onToast({ message: t('settings', 'notifSettings.saveError'), type: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const Toggle = ({ enabled, onClick }: { enabled: boolean; onClick: () => void }) => (
    <button
      onClick={onClick}
      className={`w-11 h-6 rounded-full transition-colors relative shrink-0 ${enabled ? 'bg-[#6600FF]' : 'bg-gray-200'}`}
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-md mx-auto px-5 pt-safe-header pb-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-extrabold text-[#1A1A2E] dark:text-white">
              {t('settings', 'notifSettings.title')}
            </h1>
          </div>
          {saving && <Loader2 className="w-4 h-4 text-[#6600FF] animate-spin ml-auto" />}
        </div>
      </div>
      <div className="max-w-md mx-auto px-5 py-4">
        {loading ? (
          ...
          <>
            <p className="text-sm text-gray-500 mb-4">{t('settings', 'notifSettings.subtitle')}</p>
                ...
                return (
                  <div key={item.key} className="card p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-[#1A1A2E] text-sm">{t('settings', `notifSettings.${item.labelKey}`)}</p>
                      <p className="text-xs text-gray-500">{t('settings', `notifSettings.${item.descKey}`)}</p>
                    </div>
                    <Toggle enabled={enabled} onClick={() => handleToggle(item.key)} />
                  </div>
                );
              })}
            </div>
            <div className="mt-6">
              <h2 className="text-sm font-semibold text-gray-500 uppercase mb-3">
                {t('settings', 'notifSettings.pushSection')}
              </h2>
              <div className="card p-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#6600FF]/10 flex items-center justify-center shrink-0">
                  <Smartphone className="w-5 h-5 text-[#6600FF]" />
                </div>
                <div className="flex-1">
                  <p className="font-bold text-[#1A1A2E] text-sm">{t('settings', 'notifSettings.pushEnabled')}</p>
                  <p className="text-xs text-gray-500">{t('settings', 'notifSettings.pushDesc')}</p>
                </div>
                <Toggle enabled={prefs.push_enabled} onClick={() => handleToggle('push_enabled')} />
              </div>
              {prefs.push_enabled && (
                <p className="text-xs text-gray-400 mt-2 px-2">{t('settings', 'notifSettings.pushHint')}</p>
              )}
            </div>
          </>
        ) : (
          <div className="flex flex-col items-center justify-center py-20">
            <p className="text-sm text-gray-500">{t('settings', 'notifSettings.loadError')}</p>
          </div>
        )}
      </div>
    </div>

...
export function NotificationSettingsScreen({ onBack, onToast }: NotificationSettingsScreenProps) {
  ...
  return (
    <div className="min-h-screen">
      <div className="sticky top-0 z-20 bg-white/80 dark:bg-[#14121E]/80 backdrop-blur-xl border-b border-gray-100 dark:border-white/10"><div className="max-w-md mx-auto px-5 pt-safe-header pb-4 flex items-center gap-3"><button onClick={onBack} className="w-10 h-10 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center active:scale-90 transition-transform"><ChevronLeft className="w-5 h-5 text-[#1A1A2E] dark:text-white" /></button><div className="flex items-center gap-2"><Bell className="w-5 h-5 text-[#6600FF]" /><h1 className="text-lg font-extrabold text-[#1A1A2E] dark:text-white">{t('settings', 'notifSettings.title')}</h1></div>{saving && <Loader2 className="w-4 h-4 text-[#6600FF] animate-spin ml-auto" />}</div></div>

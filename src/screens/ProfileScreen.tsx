...
export function ProfileScreen({ onEventClick, onLogin, onOrganizerDashboard, onOpenNotifications, onOpenNotificationSettings, onOpenSubscriptions, onToast }: ProfileScreenProps) {
  ...
  useEffect(() => {
    ...
    fetchPendingInvitations(user.id).then(setInvitations).catch(() => {});
  }, [user]);

  const handleRespond = async (invId: string, status: 'accepted' | 'declined') => {
    setResponding(invId);
    try { await respondToInvitation(invId, status); setInvitations(invitations.filter((i) => i.id !== invId)); refreshProfile(); } catch {} finally { setResponding(null); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#EDE8FF] pb-32">
      <div className="max-w-md mx-auto">
        ...
        <div className="px-5 mt-4"><button onClick={() => setEditOpen(true)} className="w-full card p-4 flex items-center justify-between hover:shadow-card-hover transition-all"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[#6600FF]/10 flex items-center justify-center"><Pencil className="w-5 h-5 text-[#6600FF]" /></div><div className="text-left"><p className="font-bold text-[#1A1A2E]">Modifier le profil</p><p className="text-xs text-gray-500">Nom, photo, bio, localisation</p></div></div><ChevronRight className="w-5 h-5 text-gray-400" /></button></div>
        <div className="px-5 mt-4"><div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white rounded-full shadow-sm">{user.role === 'artist' ? <Music2 className="w-4 h-4 text-[#6600FF]" /> : <Building2 className="w-4 h-4 text-[#6600FF]" />}<span className="text-sm font-semibold text-[#1A1A2E]">{user.role === 'artist' ? 'Artiste' : user.role === 'organizer' ? 'Organisateur' : 'Participant'}</span>{user.role !== 'participant' && <BadgeCheck className="w-4 h-4 text-[#6600FF]" />}</div></div>

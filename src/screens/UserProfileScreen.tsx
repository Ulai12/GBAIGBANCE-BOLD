...
import { ChevronLeft, MapPin, Calendar, BadgeCheck, Music2, Building2, User, Heart, MessageCircle, Ticket, Camera } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen pb-32">
      ...
      <div className="max-w-md mx-auto px-5 -mt-16 relative">
        <div className="flex flex-col items-center text-center">
          <div className="relative">
            <UserAvatar
              ...
              role={profile.role}
              size="2xl"
              className="w-28 h-28 border-4 border-white dark:border-[#14121E] shadow-xl"
            />
            {isSelf && (
              <button
                type="button"
                onClick={() => setPictureModalOpen(true)}
                className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-[#6600FF] text-white flex items-center justify-center shadow-md ring-2 ring-white dark:ring-[#14121E] active:scale-90 transition-transform"
          ...
          <div className="flex items-center gap-1.5 mt-3"><h1 className="text-xl font-extrabold text-[#1A1A2E] dark:text-white">{profile.name}</h1>{(profile.role === 'organizer' || profile.role === 'artist') && <BadgeCheck className="w-5 h-5 text-[#6600FF] dark:text-[#A78BFA]" />}</div>
          <div className="flex items-center gap-1.5 mt-1"><div className="w-7 h-7 rounded-full bg-[#6600FF]/10 dark:bg-[#6600FF]/25 flex items-center justify-center"><RoleIcon className="w-3.5 h-3.5 text-[#6600FF] dark:text-[#A78BFA]" /></div><span className="text-sm font-semibold text-gray-600 dark:text-gray-300">{profile.role === 'artist' ? t('common', 'artist') : profile.role === 'organizer' ? t('common', 'organizer') : t('common', 'participant')}</span></div>{profile.city && (<div className="flex items-center gap-1 mt-1.5 text-xs text-gray-500 dark:text-gray-400"><MapPin className="w-3.5 h-3.5" /><span>{COUNTRY_FLAGS[profile.country] || ''} {profile.city}, {profile.country}</span></div>)}
        </div>
        ...
        {recentEvents.length > 0 && (<div className="mt-5"><h2 className="text-sm font-bold text-[#1A1A2E] dark:text-white mb-3 flex items-center gap-2"><Ticket className="w-4 h-4 text-[#6600FF] dark:text-[#A78BFA]" />{language === 'fr' ? 'Derniers événements' : 'Recent events'}</h2><div className="space-y-2">{recentEvents.map((ev) => (<button key={ev.id} onClick={() => onEventClick(ev)} className="w-full card p-3 flex items-center gap-3 hover:shadow-card-hover transition-all text-left"><div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 bg-gray-100 dark:bg-white/5"><SmartImage src={ev.cover_url} alt={ev.title} className="w-full h-full object-cover" sizes="56px" /></div><div className="flex-1 min-w-0"><p className="font-bold text-[#1A1A2E] dark:text-white text-sm truncate">{ev.title}</p><p className="text-xs text-gray-500 dark:text-gray-400">{formatFullDate(ev.starts_at, language)} · {ev.city}</p></div><div className="flex items-center gap-1 text-xs text-gray-400"><Heart className="w-3 h-3" />{formatNumber(ev.likes_count, language)}</div></button>))}</div></div>)}

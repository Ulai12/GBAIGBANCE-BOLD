...
import { ChevronLeft, MapPin, Calendar, BadgeCheck, Music2, Building2, User, Loader2, Heart, MessageCircle, Ticket } from 'lucide-react';
...
export function UserProfileScreen({ userId, onBack, onEventClick, onToast }: UserProfileScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-[#EDE8FF] pb-32">
      <div className="relative h-48 bg-gradient-to-br from-[#6600FF]/20 via-[#9D4EDD]/15 to-[#EDE8FF]"><div className="absolute top-4 left-0 right-0 px-5"><button onClick={onBack} className="w-10 h-10 rounded-full bg-white/90 backdrop-blur flex items-center justify-center shadow-md active:scale-90 transition-transform"><ChevronLeft className="w-5 h-5 text-[#1A1A2E]" /></button></div></div>
      <div className="max-w-md mx-auto px-5 -mt-16 relative">
        <div className="flex flex-col items-center text-center"><img src={profile.avatar_url || `https://i.pravatar.cc/200?u=${profile.id}`} alt={profile.name} className="w-28 h-28 rounded-full object-cover border-4 border-white shadow-lg" /><div className="flex items-center gap-1.5 mt-3"><h1 className="text-xl font-extrabold text-[#1A1A2E]">{profile.name}</h1>{(profile.role === 'organizer' || profile.role === 'artist') && <BadgeCheck className="w-5 h-5 text-[#6600FF]" />}</div><div className="flex items-center gap-1.5 mt-1"><div className="w-7 h-7 rounded-full bg-[#6600FF]/10 flex items-center justify-center"><RoleIcon className="w-3.5 h-3.5 text-[#6600FF]" /></div><span className="text-sm font-semibold text-gray-600">{profile.role === 'artist' ? t('common', 'artist') : profile.role === 'organizer' ? t('common', 'organizer') : t('common', 'participant')}</span></div>{profile.city && (<div className="flex items-center gap-1 mt-1.5 text-xs text-gray-500"><MapPin className="w-3.5 h-3.5" /><span>{COUNTRY_FLAGS[profile.country] || ''} {profile.city}, {profile.country}</span></div>)}</div>
        <div className="grid grid-cols-2 gap-3 mt-5"><div className="card p-3 text-center"><p className="text-xl font-extrabold text-[#1A1A2E]">{followingCount}</p><p className="text-xs text-gray-500">{t('settings', 'userProfile.followingCount')}</p></div><div className="card p-3 text-center"><p className="text-xl font-extrabold text-[#1A1A2E]">{followersCount}</p><p className="text-xs text-gray-500">{t('settings', 'userProfile.followers')}</p></div></div>
        {profile.bio && (<div className="card p-4 mt-4"><p className="text-xs font-semibold text-gray-500 uppercase mb-2">{t('settings', 'userProfile.bio')}</p><p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{profile.bio}</p></div>)}
        ...
        {recentEvents.length > 0 && (<div className="mt-5"><h2 className="text-sm font-bold text-[#1A1A2E] mb-3 flex items-center gap-2"><Ticket className="w-4 h-4 text-[#6600FF]" />{language === 'fr' ? 'Derniers événements' : 'Recent events'}</h2><div className="space-y-2">{recentEvents.map((ev) => (<button key={ev.id} onClick={() => onEventClick(ev)} className="w-full card p-3 flex items-center gap-3 hover:shadow-card-hover transition-all text-left"><img src={ev.cover_url || 'https://images.pexels.com/photos/1190297/pexels-photo-1190297.jpeg?auto=compress&cs=tinysrgb&w=200'} alt={ev.title} className="w-14 h-14 rounded-xl object-cover shrink-0" /><div className="flex-1 min-w-0"><p className="font-bold text-[#1A1A2E] text-sm truncate">{ev.title}</p><p className="text-xs text-gray-500">{formatFullDate(ev.starts_at, language)} · {ev.city}</p></div><div className="flex items-center gap-1 text-xs text-gray-400"><Heart className="w-3 h-3" />{formatNumber(ev.likes_count, language)}</div></button>))}</div></div>)}
        {!isSelf && (<div className="flex gap-3 mt-5"><button onClick={handleFollow} disabled={toggling} className={`flex-1 py-3.5 rounded-full font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 ${following ? 'bg-white text-[#6600FF] border-2 border-[#6600FF]' : 'btn-purple'}`}><Heart className={`w-4 h-4 ${following ? 'fill-[#6600FF]' : ''}`} />{following ? t('settings', 'userProfile.following') : t('settings', 'userProfile.follow')}</button><button className="w-14 h-14 rounded-full bg-white shadow-md flex items-center justify-center active:scale-90 transition-transform"><MessageCircle className="w-5 h-5 text-[#6600FF]" /></button></div>)}
      </div>
    </div>
  );
}

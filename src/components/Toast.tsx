...
export function Toast({ toast, onClose }: ToastProps) {
  ...
  useEffect(() => {
    setVisible(true);
    const timer = setTimeout(() => { setVisible(false); setTimeout(() => onClose(toast.id), 300); }, 3000);
  ...
  return (
    <div className={`bg-white rounded-2xl px-4 py-3 flex items-center gap-3 shadow-lg transition-all duration-300 ${visible ? 'translate-y-0 opacity-100 scale-100' : '-translate-y-4 opacity-0 scale-90'}`}>
